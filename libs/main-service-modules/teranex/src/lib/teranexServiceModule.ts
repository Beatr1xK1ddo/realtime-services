    ...
    private handleConnection(socket: Socket) {
        ...
        socket.on('unsubscribe', ({ nodeId, ip, port }: IClientSubscribeEvent) => {
            ...
            if (devicesSubscribers && devicesSubscribers.has(deviceId)) {
                ...
                this.logger.log.info(`Socket: "${socket.id}" unsubscribed from "node: ${nodeId}" and "device ${deviceId}"`);
            }
        });
        ...
        socket.on('response', ({ nodeId, ...data }: IDeviceResponseEvent) => {
            ...
            this.logger.log.info(`Response was sent to clients with "node: ${nodeId}" and "device ${deviceId}"`);
        });
