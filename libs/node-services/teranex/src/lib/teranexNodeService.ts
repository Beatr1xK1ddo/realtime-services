import {
    ...
    IDeviceResponseEvent, NodeDeviceService, NodeServiceOptions,
...
export class TeranexNodeService extends NodeDeviceService<TeranexDevice> {
    ...
    private async handleRequest(data: IClientCmdRequestEvent) {
        ...
        try {
            ...
            this.emit('response', {nodeId: this.nodeId, ip, port, data: result} as IDeviceResponseEvent);
        } catch (error) {
            this.emit('response', {nodeId: this.nodeId, ip, port, error} as IDeviceResponseEvent);
        }
    };
    ...
    private static async createDevice(ip: string, port: number): Promise<TeranexDevice | null> {
        try {
